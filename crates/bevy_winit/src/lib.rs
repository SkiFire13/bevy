...
use bevy_ecs::system::{MainThread, NonSend};
...
impl Plugin for WinitPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(NonSend::new(WinitWindows::default()))
        ...
        app.insert_resource(create_window_reader)
            .insert_resource(NonSend::new(event_loop));
    }
}

fn change_window(
    _marker: MainThread,
    mut winit_windows: ResMut<NonSend<WinitWindows>>,
    mut windows: ResMut<Windows>,
    mut window_dpi_changed_events: EventWriter<WindowScaleFactorChanged>,
    mut window_close_events: EventWriter<WindowClosed>,
) {
    let winit_windows = winit_windows.get_mut();
...
pub fn winit_runner_with(mut app: App) {
    let mut event_loop = app
        .world
        .remove_resource::<NonSend<EventLoop<()>>>()
        .unwrap()
        .into_inner();
    ...
    app.world
        .insert_resource(NonSend::new(event_loop.create_proxy()));
    ...
    let event_handler = move |event: Event<()>,
        ...
        match event {
            ...
            event::Event::WindowEvent {
                ...
                let mut winit_windows = world.resource_mut::<NonSend<WinitWindows>>();
                let winit_windows = winit_windows.get_mut();
